...
int main()
{
    ...
    //play is initialized as 'Y' so 1st loop can initially run
    //(while play is equal to Y || y)
    char play = 'Y';
    
    cout    << "Hello, are you ready to play the hi-lo game? I am thinking of a"
            << " number, enter a guess, and I will tell you if it is too high, "
            << "too low, or just right.\n";
    
    while (play == 'Y' || play == 'y') {
        cout << "\nI am thinking of a number between 0 and 100.\n";
        
        /*initializes guess number to -1, that way the 2nd while loop can
         initially run (while (randnum != guess)) and there is no chance that
         the loop starts with a guess that is equal to the random number seeded
         by the computer (-1 is out of the range from 0-100).*/
        ...
        while (randnum != guess){
            ...
            //use else if try_num == 1, so that it outputs "You won in 1 guess!"
            //instead of "You won in 1 guesses!" (Correct grammar.)
            else if (try_num == 1){
